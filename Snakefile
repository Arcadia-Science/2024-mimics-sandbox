...
rule download_human_uniprot_proteome:
    output: faa="inputs/uniprot/human/UP000005640_proteome.fasta.gz"
    shell:
        """
...
rule download_viral_IL17_mimics:
    ...
    """
    output: faa = "inputs/uniprot/viral/IL-17_taxonomy_id_3A10239.fasta.gz"
    shell:
        """
...
rule filter_proteins_by_length:
    ...
    """
    input: rules.combine_all_proteins.output.faa
    output: faa="outputs/input_proteins/length_filtered_proteins.faa.gz"
    conda: "envs/seqkit.yml"
    shell:
        """
